...
bool Option::Initialize(void)
{
	...
	// position
	const D3DXVECTOR2 MENU_OFFSETT = D3DXVECTOR2(300.0f, 10.0f);

	const D3DXVECTOR2 DEF_LOGO_POS = D3DXVECTOR2(DEFAULT_SCREEN_WIDTH * 0.5f - 170.0f, 160.f);
	volume_logo_->__position(D3DXVECTOR2(DEFAULT_SCREEN_WIDTH * 0.5f/* - 340.0f*/, DEF_LOGO_POS.y - 60.0f));
	bgm_volume_->__position(D3DXVECTOR2(DEF_LOGO_POS.x, DEF_LOGO_POS.y), 200.0f);
	se_volume_->__position(D3DXVECTOR2(DEF_LOGO_POS.x, DEF_LOGO_POS.y + DEFAULT_MENU_FRAME_SIZE.y + MENU_OFFSETT.y), 200.0f);

//	const D3DXVECTOR2 DEF_MENU_POS = D3DXVECTOR2(DEFAULT_SCREEN_WIDTH * 0.5f - 250, 360.0f);
	const D3DXVECTOR2 KEY_MENU_OFFSETT = D3DXVECTOR2(250.0f, 10.0f);
	const D3DXVECTOR2 DEF_MENU_POS = D3DXVECTOR2(DEFAULT_SCREEN_WIDTH * 0.5f - 100.0f, 360.0f);
	keyconfig_logo_->__position(D3DXVECTOR2(DEFAULT_SCREEN_WIDTH * 0.5f/* - 300.0f*/, 300.f));
	key_config_ok_->__position(D3DXVECTOR2(DEF_MENU_POS.x, DEF_MENU_POS.y), KEY_MENU_OFFSETT.x);
	key_config_cancel_->__position(D3DXVECTOR2(DEF_MENU_POS.x, DEF_MENU_POS.y + DEFAULT_MENU_FRAME_SIZE.y + KEY_MENU_OFFSETT.y), KEY_MENU_OFFSETT.x);
	key_config_special_->__position(D3DXVECTOR2(DEF_MENU_POS.x, DEF_MENU_POS.y + (DEFAULT_MENU_FRAME_SIZE.y + KEY_MENU_OFFSETT.y) * 2), KEY_MENU_OFFSETT.x);
	key_config_pause_->__position(D3DXVECTOR2(DEF_MENU_POS.x, DEF_MENU_POS.y + (DEFAULT_MENU_FRAME_SIZE.y + KEY_MENU_OFFSETT.y) * 3), KEY_MENU_OFFSETT.x);

	// decide
	option_decide_->__position(D3DXVECTOR2(DEF_MENU_POS.x, DEF_MENU_POS.y + (DEFAULT_MENU_FRAME_SIZE.y + KEY_MENU_OFFSETT.y) * 4), KEY_MENU_OFFSETT.x);
